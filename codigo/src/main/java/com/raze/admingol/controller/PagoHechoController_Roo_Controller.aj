// WARNING: DO NOT EDIT THIS FILE. THIS FILE IS MANAGED BY SPRING ROO.
// You may push code into the target .java compilation unit if you wish to edit any member(s).

package com.raze.admingol.controller;

import com.raze.admingol.controller.PagoHechoController;
import com.raze.admingol.domain.ControlPago;
import com.raze.admingol.domain.PagoHecho;
import com.raze.admingol.domain.Usuario;
import java.io.UnsupportedEncodingException;
import java.util.ArrayList;
import java.util.List;
import javax.servlet.http.HttpServletRequest;
import javax.validation.Valid;
import org.joda.time.format.DateTimeFormat;
import org.springframework.context.i18n.LocaleContextHolder;
import org.springframework.ui.Model;
import org.springframework.validation.BindingResult;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.util.UriUtils;
import org.springframework.web.util.WebUtils;

privileged aspect PagoHechoController_Roo_Controller {
    
    @RequestMapping(method = RequestMethod.POST, produces = "text/html")
    public String PagoHechoController.create(@Valid PagoHecho pagoHecho, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, pagoHecho);
            return "pagohechoes/create";
        }
        uiModel.asMap().clear();
        pagoHecho.persist();
        return "redirect:/pagohechoes/" + encodeUrlPathSegment(pagoHecho.getId().toString(), httpServletRequest);
    }
    
    @RequestMapping(params = "form", produces = "text/html")
    public String PagoHechoController.createForm(Model uiModel) {
        populateEditForm(uiModel, new PagoHecho());
        List<String[]> dependencies = new ArrayList<String[]>();
        if (ControlPago.countControlPagoes() == 0) {
            dependencies.add(new String[] { "controlpago", "controlpagoes" });
        }
        uiModel.addAttribute("dependencies", dependencies);
        return "pagohechoes/create";
    }
    
    @RequestMapping(value = "/{id}", produces = "text/html")
    public String PagoHechoController.show(@PathVariable("id") Long id, Model uiModel) {
        addDateTimeFormatPatterns(uiModel);
        uiModel.addAttribute("pagohecho", PagoHecho.findPagoHecho(id));
        uiModel.addAttribute("itemId", id);
        return "pagohechoes/show";
    }
    
    @RequestMapping(produces = "text/html")
    public String PagoHechoController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("pagohechoes", PagoHecho.findPagoHechoEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) PagoHecho.countPagoHechoes() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("pagohechoes", PagoHecho.findAllPagoHechoes(sortFieldName, sortOrder));
        }
        addDateTimeFormatPatterns(uiModel);
        return "pagohechoes/list";
    }
    
    @RequestMapping(method = RequestMethod.PUT, produces = "text/html")
    public String PagoHechoController.update(@Valid PagoHecho pagoHecho, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, pagoHecho);
            return "pagohechoes/update";
        }
        uiModel.asMap().clear();
        pagoHecho.merge();
        return "redirect:/pagohechoes/" + encodeUrlPathSegment(pagoHecho.getId().toString(), httpServletRequest);
    }
    
    @RequestMapping(value = "/{id}", params = "form", produces = "text/html")
    public String PagoHechoController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, PagoHecho.findPagoHecho(id));
        return "pagohechoes/update";
    }
    
    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE, produces = "text/html")
    public String PagoHechoController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        PagoHecho pagoHecho = PagoHecho.findPagoHecho(id);
        pagoHecho.remove();
        uiModel.asMap().clear();
        uiModel.addAttribute("page", (page == null) ? "1" : page.toString());
        uiModel.addAttribute("size", (size == null) ? "10" : size.toString());
        return "redirect:/pagohechoes";
    }
    
    void PagoHechoController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("pagoHecho_fechapago_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
        uiModel.addAttribute("pagoHecho_fechavencimiento_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
        uiModel.addAttribute("pagoHecho_fechacreacion_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
        uiModel.addAttribute("pagoHecho_fechamodificacion_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
    }
    
    void PagoHechoController.populateEditForm(Model uiModel, PagoHecho pagoHecho) {
        uiModel.addAttribute("pagoHecho", pagoHecho);
        addDateTimeFormatPatterns(uiModel);
        uiModel.addAttribute("controlpagoes", ControlPago.findAllControlPagoes());
        uiModel.addAttribute("usuarios", Usuario.findAllUsuarios());
    }
    
    String PagoHechoController.encodeUrlPathSegment(String pathSegment, HttpServletRequest httpServletRequest) {
        String enc = httpServletRequest.getCharacterEncoding();
        if (enc == null) {
            enc = WebUtils.DEFAULT_CHARACTER_ENCODING;
        }
        try {
            pathSegment = UriUtils.encodePathSegment(pathSegment, enc);
        } catch (UnsupportedEncodingException uee) {}
        return pathSegment;
    }
    
}
